	...
	*/
	class Clock: public solver<int>{
		...
		public:
			//variables to store the number of hours on dial, the initial hour and the final hour
			int totalNumber, starting, ending;
			...
			*/
			vector<int> newConfig(int config){
				...
				//add the left move and right move to the vector
				final.push_back(previous);
	...
	int main(int argc, char const *argv[]){
		...
		int end=atoi(argv[3]);
		Clock c(numberOfHours, start, end);
